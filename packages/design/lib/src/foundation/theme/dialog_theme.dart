...
class AppDialogTheme extends ThemeExtension<AppDialogTheme> {
  AppDialogTheme({required this.barrierColor});
  ...
  final padding = const EdgeInsets.symmetric(
    horizontal: 16,
  ).copyWith(top: 36, bottom: 20);
  final shape = RoundedRectangleBorder(borderRadius: BorderRadius.circular(8));

  @override
  ThemeExtension<AppDialogTheme> copyWith({Color? barrierColor}) =>
      AppDialogTheme(barrierColor: barrierColor ?? this.barrierColor);

  @override
  ThemeExtension<AppDialogTheme> lerp(
    ThemeExtension<AppDialogTheme>? other,
    double t,
  ) =>
      other is! AppDialogTheme
          ? this
          : AppDialogTheme(
  ...
  static AppDialogTheme of(BuildContext context) {
    return AppDialogTheme(
      barrierColor: context.tokens.color.colorsGrey10.withOpacity(.5),
    );
  }
}
